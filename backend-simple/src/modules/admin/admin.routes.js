...
const { authGuard, adminGuard } = require('../../common/auth-guard');
...
router.get('/stats', authGuard, adminGuard, (req, res) => {
  const totalUsers = db.users.filter(u => u.deleted_at === null).length;
...
// Admin add track
router.post('/tracks', authGuard, adminGuard, (req, res, next) => {
  try {
...
router.post('/notifications', authGuard, adminGuard, (req, res, next) => {
  try {
